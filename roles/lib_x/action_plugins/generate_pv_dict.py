"""
Ansible action plugin to generate pv and pvc dictionaries
"""
import sys
...
class ActionModule(ActionBase):
    ...
    def build_common(self, varname=None):
        '''Retrieve common variables for each pv type'''
    ...
    def build_pv_nfs(self, varname=None):
        ...
        if host:
            ...
            if groups and 'oo_nfs_to_config' in groups and len(groups['oo_nfs_to_config']) > 0:
    ...
    def build_pv_dict(self, varname=None):
        ...
        if kind:
            ...
            if create_pv and self._templar.template(create_pv):
                if kind == 'nfs':
                    pv = self.build_pv_nfs(varname=varname)

                elif kind == 'openstack':
                    pv = self.build_pv_openstack(varname=varname)

                elif kind == 'glusterfs':
                    pv = self.build_pv_glusterfs(varname=varname)
